...
Этот Telegram-бот предназначен для получения информации о карточках в Planka **версии 1.x** на конкретную дату. Инструмент позволяет легко контролировать назначенные дела и не пропускать ничего важного, даже если задач и досок очень много. Просто нажмите кнопку в боте, чтобы мгновенно получить актуальный список задач.

Если вы используете **Planka версии 2.x**, переходите на [Bot_checker_date_planka_2.0](https://github.com/john-gear/bot_checker_date_planka_2.0).
